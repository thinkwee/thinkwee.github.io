...
## 当我们讨论推断时，我们在推断什么
-	当然我们推断的是模型的参数，但是MCMC方法只能已知分布，采样出一系列符合已知分布的样本，通过样本估计统计量。所以在LDA当中，为什么我们可以用MCMC推断LDA模型的参数？具体而言两个问题：
	-	我们不知道参数，自然是不知道分布的，哪来的已知分布进行Gibbs采样？
	-	我们采样得到的样本只能得到统计量，怎么得到具体的参数？
-	之前已经解释了第一个问题，在Gibbs采样公式的推断中，我们用Dirichlet的参数（人为设置超参）来近似已知分布的参数，因此我们没用已知的分布，而是用近似的分布。
-	第二个问题，其实在非参贝叶斯推断中，很多情况下统计量（各阶矩）就是模型的参数，例如高斯分布的参数，均值和方差，均值就是一阶矩，方差就是二阶矩和一阶矩的函数。这个问题在LDA里比较特殊。我们得到的样本和统计量是什么？样本是每个词所属的主题。统计量是每个主题下词频数和每个文档下的主题频数，显然这两个频数归一化就得到LDA的参数，即文档到主题和主题到词的多项式分布。
-	因此传统的MCMC用法是已知分布，分布的函数（统计量）不好算，用MCMC生成样本算统计量。在非参贝叶斯推断里MCMC的用法是未知分布，但是统计量好算，甚至能够通过统计量反推参数，因此就用近似分布来生成样本，进而推断参数。当然单纯的近似分布不一定好使，这里还利用了贝叶斯框架，用数据知识不断的对先验补充，不断的调整近似分布使得其接近真实分布。
