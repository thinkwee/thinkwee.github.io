...
# STRUCTURED NEURAL SUMMARIZATION
-	将GNN引入生成式摘要的一个很自然的想法就是先用sequence encoder得到token representations，然后再构建token之间的关系，建图，将图和token representations送入GNN，得到新的表示。
...
-	作者使用的GNN是Gated Graph Neural Network，不同类型的边用不同的线性层做一次变换，再aggregate求和，之后用GRU做update，如果要用到的整张图的表示，就对每个节点做一次gated update，$\sigma(w(h_v^T))$。
...
-	作者也没有对结果做太多分析，因为论文做了三个数据集，其余两个是代码摘要，有比较自然的结构化数据，因此表现很好，在CNNDM这种纯自然语言数据集上表现并不是特别亮眼。在消融实验中值得注意的是即便不添加指代信息，仅仅是让GNN处理句子结构，表现也比LSTM要好。
-	总而言之直接以词级别为节点做GNN太过粗暴了，对于摘要这种文档级别理解任务，许多更高层次的边关系也没有利用。但是在目前没有更好的与decoder相结合的前提下，这样的做法至少是可行的。